<template>
    <div v-if="lang.toLowerCase() === langFromQuery" class="rounded-lg my-5 grid gap-6 md:grid-cols-2 grid-cols-1"
      ...
      <div class="col-span-1">
        <section class="flex justify-between flex-row p-2 text-black dark:bg-gray-700 bg-gray-200 rounded-t-lg">
          ...
          <div class="relative">
            <button v-show="!showDropDown" @click="showDropDown = true"
              ...
              <svg class="w-5 h-5 mt-1 dark:text-white text-gray-700" xmlns="http://www.w3.org/2000/svg"
            ...
            <button v-show="showDropDown" @click="showDropDown = false"
              class="flex flex-row px-3 py-1 rounded-md dark:bg-gray-800 dark:text-gray-300 text-gray-800 border-0">
              <span class="mr-1">{{ lang }}</span>
              <svg class="w-5 h-5 mt-1 text-white text-gray-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                fill="currentColor">
                <path fill-rule="evenodd"
                  d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                  clip-rule="evenodd" />
            ...
            <!-- Dropdown menu -->
            <div v-show="showDropDown" ref="dropDownItems"
              class="absolute right-0 py-2 mt-2 rounded-md shadow-xl z-10 dark:bg-gray-800 bg-gray-200 text-gray-800">
              <div :value="item.name" v-for="(item, index) in labraries"
                v-show="item.name.toLowerCase() !== langFromQuery"
                class="block px-4 py-2 text-sm hover:bg-gray-700 dark:text-gray-300 cursor-pointer" :key="index"
